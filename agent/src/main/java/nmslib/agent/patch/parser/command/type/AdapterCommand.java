...
public final class AdapterCommand implements Command {
    ...
    public void execute(final CommandContext ctx, final String[] args) {
        if (args.length < 1) {
            throw new IllegalStateException("missing args for 'adapter'; "
                    + "usage: adapter <type>");
        }
        ...
        if (patchClass == null) {
            throw new IllegalStateException("cannot execute 'adapter' without 'class'");
        }

        patchClass.adapter(args[0]);
    }
